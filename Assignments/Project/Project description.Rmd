...
3 points for properly joined, related data;
3 points for presenting a summary table of the final sample selected for analysis that formats well on a letter size page;
3 points for preparing at least one figure that uses two of the following ggplot features (color, size, shape, or facet);
3 points for adherence to report-style format (organization, format, narrative, captions, citation, references, spelling, grammar); 
3 point for coding style, proper knitting;
6 points for correctness; 
9 points for the preparation, diagnoses, and interpretation of at least one multiple linear regression model that includes at least one continuous variable and one factor variable;

Considerable latitude will be given for "correctness," as I expect you to each make different choices. I encourage you to focus on the grading rubric and not attempting an "optimal" solution. Students making very different choices could both get perfect scores. 

You are expected to make judgments. Please make reasoned, disciplined judgments. Judgments made for convenience are not disciplined.

The format of the project is a REPORT, which is very different than your assignments. You may include code if it helps you explain your choices. Otherwise, narrative, figures, and tables are preferred formats for reports.  

You can markdown your report to either html or pdf format. Your report should print on no more than 10 pages. 

Please turn in both your report (html or pdf) and .Rmd file. 

You do no not have to model time effects for this project. You can if you like but it is not expected. 
